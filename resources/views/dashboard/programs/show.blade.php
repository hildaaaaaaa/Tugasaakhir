...
<nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
  ...
  <form class="col-lg-8 mt-5 ms-5" action="/dashboard/programs/{{ $program->id }}" method="post">
    ...
    <div class=" mb-3">
      ...
      <input type="text" class="form-control" id="proker" name="proker" value="{{ old('proker', $program->proker) }}" disabled>
      @error('proker')
    ...
    <div class="mb-3">
      ...
      <input type="date" class="form-control" id="tanggal" name="tanggal" value="{{ old('tanggal', $program->tanggal) }}" disabled>
      @error('tanggal')
        <div class="text-danger">{{ $message }}</div>
      @enderror
    </div>
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="deskripsi" name="deskripsi" value="{{ old('deskripsi', $program->deskripsi) }}" disabled>
      @error('deskripsi')
        <div class="text-danger">{{ $message }}</div>
      @enderror
    </div>
